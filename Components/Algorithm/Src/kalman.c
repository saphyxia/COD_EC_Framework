...
void Kalman_Filter_Init(KalmanFilter_Info_TypeDef *kf,uint8_t xhatSize,uint8_t uSize,uint8_t zSize)
{
    ...
    kf->Memory.xhat = (float *)user_malloc(kf->sizeof_float * xhatSize);
    memset(kf->Memory.xhat, 0, kf->sizeof_float * xhatSize);
    Matrix_Init(&kf->Mat.xhat, kf->xhatSize, 1, (float *)kf->Memory.xhat);

    /* Initializes the xhatminus */
    kf->Memory.xhatminus = (float *)user_malloc(kf->sizeof_float * xhatSize);
    memset(kf->Memory.xhatminus, 0, kf->sizeof_float * xhatSize);
    Matrix_Init(&kf->Mat.xhatminus, kf->xhatSize, 1, (float *)kf->Memory.xhatminus);

    /* Initializes the measurement vector */
    kf->Memory.z = (float *)user_malloc(kf->sizeof_float * zSize);
    memset(kf->Memory.z, 0, kf->sizeof_float * zSize);
    ...
    if (kf->uSize != 0)
    {
        ...
        kf->Memory.u = (float *)user_malloc(kf->sizeof_float * uSize);
        memset(kf->Memory.u, 0, kf->sizeof_float * uSize);
        Matrix_Init(&kf->Mat.u, kf->uSize, 1, (float *)kf->Memory.u);

        /* Initializes the control Matrix */  
        kf->Memory.B = (float *)user_malloc(kf->sizeof_float * xhatSize * uSize);
        memset(kf->Memory.B, 0, kf->sizeof_float * xhatSize * uSize);
        Matrix_Init(&kf->Mat.B, kf->xhatSize, kf->uSize, (float *)kf->Memory.B);
    }

    /* Initializes the state transition Matrix */ 
    kf->Memory.A = (float *)user_malloc(kf->sizeof_float * xhatSize * xhatSize);
    memset(kf->Memory.A, 0, kf->sizeof_float * xhatSize * xhatSize);
    Matrix_Init(&kf->Mat.A, kf->xhatSize, kf->xhatSize, (float *)kf->Memory.A);

    kf->Memory.AT = (float *)user_malloc(kf->sizeof_float * xhatSize * xhatSize);
    memset(kf->Memory.AT, 0, kf->sizeof_float * xhatSize * xhatSize);
    Matrix_Init(&kf->Mat.AT, kf->xhatSize, kf->xhatSize, (float *)kf->Memory.AT);

    /* Initializes the measurement Matrix */ 
    kf->Memory.H = (float *)user_malloc(kf->sizeof_float * zSize * xhatSize);
    memset(kf->Memory.H, 0, kf->sizeof_float * zSize * xhatSize);
    Matrix_Init(&kf->Mat.H, kf->zSize, kf->xhatSize, (float *)kf->Memory.H);

    kf->Memory.HT = (float *)user_malloc(kf->sizeof_float * xhatSize * zSize);
    memset(kf->Memory.HT, 0, kf->sizeof_float * xhatSize * zSize);
    Matrix_Init(&kf->Mat.HT, kf->xhatSize, kf->zSize, (float *)kf->Memory.HT);

    /* Initializes the posteriori covariance Matrix */
    kf->Memory.P = (float *)user_malloc(kf->sizeof_float * xhatSize * xhatSize);
    memset(kf->Memory.P, 0, kf->sizeof_float * xhatSize * xhatSize);
    Matrix_Init(&kf->Mat.P, kf->xhatSize, kf->xhatSize, (float *)kf->Memory.P);

    /* Initializes the priori covariance Matrix */
    kf->Memory.Pminus = (float *)user_malloc(kf->sizeof_float * xhatSize * xhatSize);
    memset(kf->Memory.Pminus, 0, kf->sizeof_float * xhatSize * xhatSize);
    Matrix_Init(&kf->Mat.Pminus, kf->xhatSize, kf->xhatSize, (float *)kf->Memory.Pminus);

    /* Initializes the process noise covariance Matrix */  
    kf->Memory.Q = (float *)user_malloc(kf->sizeof_float * xhatSize * xhatSize);
    memset(kf->Memory.Q, 0, kf->sizeof_float * xhatSize * xhatSize);
    Matrix_Init(&kf->Mat.Q, kf->xhatSize, kf->xhatSize, (float *)kf->Memory.Q);

    /* Initializes the measurement noise covariance Matrix */
    kf->Memory.R = (float *)user_malloc(kf->sizeof_float * zSize * zSize);
    memset(kf->Memory.R, 0, kf->sizeof_float * zSize * zSize);
    Matrix_Init(&kf->Mat.R, kf->zSize, kf->zSize, (float *)kf->Memory.R);

    /* Initializes the kalman gain */  
    kf->Memory.K = (float *)user_malloc(kf->sizeof_float * xhatSize * zSize);
    memset(kf->Memory.K, 0, kf->sizeof_float * xhatSize * zSize);
    Matrix_Init(&kf->Mat.K, kf->xhatSize, kf->zSize, (float *)kf->Memory.K);

    /* Initializes the K_denominator (K_denominator = H Pminus HT + R) */  
    kf->Memory.K_denominator = (float *)user_malloc(kf->sizeof_float * kf->xhatSize * kf->xhatSize);
    memset(kf->Memory.K_denominator, 0, kf->sizeof_float * kf->xhatSize * kf->xhatSize);
    ...
    kf->Memory.cache_matrix[0] = (float *)user_malloc(kf->sizeof_float * kf->xhatSize * kf->xhatSize);
    memset(kf->Memory.cache_matrix[0],0,kf->sizeof_float * kf->xhatSize * kf->xhatSize);
    Matrix_Init(&kf->Mat.cache_matrix[0], kf->xhatSize, kf->xhatSize, (float *)kf->Memory.cache_matrix[0]);

    kf->Memory.cache_matrix[1] = (float *)user_malloc(kf->sizeof_float * kf->xhatSize * kf->xhatSize);
    memset(kf->Memory.cache_matrix[1],0,kf->sizeof_float * kf->xhatSize * kf->xhatSize);
    ...
    kf->Memory.cache_vector[0] = (float *)user_malloc(kf->sizeof_float * kf->xhatSize);
    memset(kf->Memory.cache_vector[0],0,kf->sizeof_float * kf->xhatSize);
    Matrix_Init(&kf->Mat.cache_vector[0], kf->xhatSize, 1, (float *)kf->Memory.cache_vector[0]);

    kf->Memory.cache_vector[1] = (float *)user_malloc(kf->sizeof_float * kf->xhatSize);
    memset(kf->Memory.cache_vector[1],0,kf->sizeof_float * kf->xhatSize);
...
static void Kalman_Filter_Measurement_Update(KalmanFilter_Info_TypeDef *kf)
{
    ...
    memcpy(kf->Memory.z, kf->MeasuredVector, kf->sizeof_float * kf->zSize);
...
float *Kalman_Filter_Update(KalmanFilter_Info_TypeDef *kf)
{
    ...
    memcpy(kf->Output, kf->Memory.xhat, kf->sizeof_float * kf->xhatSize);
